...
object SteamVoiceProtocol {
    ...
    fun decode(data: ByteArray, length: Int): SteamVoicePacket? {
        if (length < headerSize || data.copyOfRange(0, 4).decodeToString() != "SV01" || data[4].toInt() != version || (data[5].toInt() != codecOpus && data[5].toInt() != codecPcm)) return null
