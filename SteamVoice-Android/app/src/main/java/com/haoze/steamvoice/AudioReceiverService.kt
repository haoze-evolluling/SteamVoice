    ...
    private fun receiveLoop() {
        ...
        try {
            ...
            while (!stopRequested && !Thread.currentThread().isInterrupted) {
                ...
                if (!gotPacket) { if (System.nanoTime() - lastFeedback > 200_000_000L) { sendFeedback(lastAddress, lastPort, activeSession, highest, receivedCount, lostCount, buffer.queuedPackets(), settings.bitrateKbps * 1000); lastFeedback = System.nanoTime() }; continue }
                received++
                ...
                while (true) {
                    ...
                    when (item) {
                        is PacketJitterBuffer.Item.Packet -> {
                            if (fecPending) { val recovered = if ((item.value.flags and 1) != 0) OpusNative.decode(decoderHandle, item.value.opus, true) else OpusNative.decodePlc(decoderHandle, 480); recovered?.let { track.write(it, 0, it.size, AudioTrack.WRITE_BLOCKING) }; fecPending = false }
                ...
                if (System.nanoTime() - lastFeedback > 200_000_000L) { sendFeedback(lastAddress, lastPort, activeSession, highest, receivedCount, lostCount, buffer.queuedPackets(), settings.bitrateKbps * 1000); lastFeedback = System.nanoTime() }
            }
        } catch (e: Exception) {
