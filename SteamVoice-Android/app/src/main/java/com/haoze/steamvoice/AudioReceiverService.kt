...
class AudioReceiverService : Service() {
    ...
    private fun registerService() { val settings = runBlocking { SettingsRepository(this@AudioReceiverService).settings.first() }; nsd=getSystemService(Context.NSD_SERVICE) as NsdManager; val manufacturer=android.os.Build.MANUFACTURER.trim(); val model=android.os.Build.MODEL.trim(); val friendly=listOf(manufacturer,model).filter { it.isNotEmpty() }.distinct().joinToString(" ").ifEmpty { "Android device" }; val info=NsdServiceInfo().apply { serviceName="SteamVoice-$friendly"; serviceType="_steamvoice._udp."; port=SteamVoiceProtocol.port; setAttribute("codec", "opus"); setAttribute("sample_rate", SteamVoiceProtocol.sampleRate.toString()); setAttribute("channels", SteamVoiceProtocol.channels.toString()); setAttribute("bitrate", (settings.bitrateKbps * 1000).toString()); setAttribute("frame_ms", SteamVoiceProtocol.frameMilliseconds.toString()) }; registration=object:NsdManager.RegistrationListener { override fun onServiceRegistered(i:NsdServiceInfo){ Log.i(TAG,"advertising ${i.serviceName}") }; override fun onRegistrationFailed(i:NsdServiceInfo,e:Int){ Log.e(TAG,"NSD registration failed: $e") }; override fun onServiceUnregistered(i:NsdServiceInfo){}; override fun onUnregistrationFailed(i:NsdServiceInfo,e:Int){ Log.e(TAG,"NSD unregistration failed: $e") } }; nsd?.registerService(info,NsdManager.PROTOCOL_DNS_SD,registration) }
    private fun receiveLoop() { val settings = runBlocking { SettingsRepository(this@AudioReceiverService).settings.first() }; val track=newTrack(); val buffer=PacketJitterBuffer(targetPackets = (settings.latency.targetMs / SteamVoiceProtocol.frameMilliseconds).coerceAtLeast(1)); socket=DatagramSocket(SteamVoiceProtocol.port); val bytes=ByteArray(MAX_UDP_PACKET); var received=0L; var decoded=0L; var opus: OpusDecoder? = null; try { while (!stopRequested && !Thread.currentThread().isInterrupted) { val datagram=DatagramPacket(bytes,bytes.size); socket?.receive(datagram); received++; val packet=SteamVoiceProtocol.decode(datagram.data,datagram.length); if (packet == null) { Log.w(TAG,"invalid UDP packet length=${datagram.length}"); continue }; if (opus == null) opus = OpusDecoder(); decoded++; buffer.offer(packet); while(true) { val encoded=buffer.take() ?: break; val pcm=opus.decode(encoded.opus, encoded.frameMilliseconds) ?: continue; val written=track.write(pcm,0,pcm.size,AudioTrack.WRITE_BLOCKING); if (written < 0) Log.e(TAG,"AudioTrack.write failed: $written") } } } catch (e: Exception) { if (!stopRequested) Log.e(TAG,"receiver loop stopped; Opus decoder unavailable",e) } finally { opus?.release(); Log.i(TAG,"receiver stopped received=$received decoded=$decoded"); track.stop(); track.release() } }
