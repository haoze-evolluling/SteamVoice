    ...
    private fun receiveLoop() {
        ...
        try {
            while (!stopRequested && !Thread.currentThread().isInterrupted) {
                ...
                buffer.offer(packet)
                while (true) {
                    val encoded = buffer.take() ?: break
                    val pcm = if (encoded.codec == SteamVoiceProtocol.codecPcm) encoded.opus else {
                        if (opus == null) opus = OpusDecoder()
                        opus!!.decode(encoded.opus, encoded.frameMilliseconds)
                    } ?: continue
