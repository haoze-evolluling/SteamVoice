...
- 多设备外放：电脑端可同时连接多台 Android 设备同步外放，各连接独立进行码率自适应；接收端停止反馈时会在设备行提示“接收端无响应”。
...
3. 在 Windows 上打开 SteamVoice Desktop，扫描并选择 Android 设备，然后连接。可依次连接多台设备同时外放（同时连接的设备需使用相同的音频帧时长）。
