...
- 多设备声道路由：连接多台设备时，可在桌面端为每台设备独立指定播放声道（立体声 / 左声道 / 右声道），实现例如 A 设备播左声道、B 设备播右声道的分体外放，切换实时生效。
...
- 固定码率：可选择 64/96/128/192 kbps，发送端在播放期间保持所选码率不变。
...
- **Windows 端提示 Opus 编码器不可用**：确认以 `steamvoice_opus nolibopusfile` 构建标签启动，并让 `pkg-config --exists opus` 成功。
...
- `SVCT`：接收端反馈（丢包/积压/队列），同时附带校准阶段与时钟偏差、往返延迟，驱动两端的同步校准动画。
- `SVCS`：音频设置同步。
